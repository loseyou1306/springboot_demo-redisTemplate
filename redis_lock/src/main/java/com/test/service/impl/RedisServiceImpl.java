...
public class RedisServiceImpl implements RedisService{
     ...
     */
	@Override
	public boolean cacheValueIfExist(String k, String v, long time) {
		  String key = KEY_PREFIX_VALUE + k;
		  Boolean result =false;
	        try {
	            ValueOperations<String, String> valueOps = redisTemplate.opsForValue();
	            result = valueOps.setIfAbsent(key, v);
	            if(time > 0){
	            	redisTemplate.expire(key, time, TimeUnit.SECONDS);
	            }
	        } catch (Throwable t) {
	 ...
	 */
	@Override
	public boolean cacheValueIfExist(String k, String v) {
		return cacheValueIfExist(k,v,-1);
	}
}